...
export default function Navbar() {
  ...
  const getHref = (href: string) => {
    // If we are in B1 context (URL starts with /b1)
    if (pathname.startsWith("/b1") || level === "B1") {
      if (href === "/") return "/b1";
      return `/b1${href}`;
    }
    // A2 context
  ...
  const handleLevelChange = (newLevel: "A2" | "B1") => {
    ...
    if (newLevel === "B1") {
      ...
      else targetPath = "/a2"; // redirect to A2 dashboard
    }
            ...
            <Link href="/" className="flex items-center gap-2 cursor-pointer group">
              ...
              <div className="flex flex-col">
                <span className="text-lg font-bold text-gray-900 leading-none group-hover:text-goethe-purple transition-colors">
                  GoetheForge <span className="text-goethe-purple">{isPortal ? "Portal" : level}</span>
                </span>
                <span className="text-xs text-gray-500 font-medium mt-0.5">
                  {isPortal ? "Materi & Simulasi A1-B2" : `Persiapan Ujian Goethe ${level}`}
                </span>
              </div>
            </Link>
          </div>
            ...
            <div className="hidden lg:flex items-center gap-2 xl:gap-3">
              ...
              <div className="flex items-center gap-0.5 xl:gap-1">
                {NAV_ITEMS.map((item) => {
                  ...
                  const isActive = pathname === targetHref || (targetHref !== "/" && targetHref !== "/b1" && targetHref !== "/a2" && pathname?.startsWith(targetHref));
                  return (
                    <Link
                      key={item.href}
                      href={targetHref}
